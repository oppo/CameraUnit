...
public abstract class BaseFeatureVideoHdr implements IFeature {
    @Override
    public String getFeatureName(Context appContext) {
        return appContext.getString(R.string.config_name_video_hdr);
    }
    ...
    public List<String> getSupportFeatureDisplayValues(Context appContext, ConfigureBean configure) {
        ...
        for (String supportFeatureSubValue : subValues) {
            if (Constant.HdrMode.HDR_AUTO.equals(supportFeatureSubValue)) {
                displayLists.add(appContext.getString(R.string.display_hdr_auto));
            } else if (Constant.HdrMode.HDR_OFF.equals(supportFeatureSubValue)) {
                displayLists.add(appContext.getString(R.string.display_hdr_off));
            } else if (Constant.HdrMode.HDR_ON.equals(supportFeatureSubValue)) {
